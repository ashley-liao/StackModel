...
def main(url, html, tldlist):
    ...
    url_class = URL_check(url, tldlist)
    ...
    print('Feature Extraction completed, average time taken:', (time.time() - start_time))
    ...
    print('Average runtime: ', (time.time() - start_time))

    pred_df = pd.DataFrame({'URL':url, 'y_pred':y_pred})
    print(y_pred)
    return y_pred
...
def receive_data():
    data = request.json
    result = main(data['url'], data['html'], 'tld.csv')
    
    # Convert ndarray to list if result is an ndarray
    if isinstance(result, np.ndarray):
        result = result.tolist()
    
    return jsonify({"status": "success", "result": result})
app.run(debug=True, port=4765)